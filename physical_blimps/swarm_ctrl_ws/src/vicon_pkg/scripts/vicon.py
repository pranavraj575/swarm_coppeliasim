...
class Vicon:
    def __init__(self, topics, topic_global='/vicon/', queue_len=3):
        """
    ...
    def update(self, obj_ids, pos_samples=1, vel_samples=1, acc_samples=1):
        ...
        for obj_id in obj_ids:
            ...
            self.blimps[obj_id]['info']['linear']['pos'] = np.mean(
                [object_info['pos_hist'][-i] for i in range(1, pos_samp + 1)], axis=0)
            ...
            if len(object_info['time_hist']) > 1 and vel_samples > 0:
                ...
                self.blimps[obj_id]['info']['linear']['vel'] = np.mean([
                    (object_info['pos_hist'][-i] - object_info['pos_hist'][-i - 1])/(
                            object_info['time_hist'][-i] - object_info['time_hist'][-i - 1])
                    for i in range(1, vel_samp + 1)], axis=0)

                self.blimps[obj_id]['info']['angular']['vel'] = self.angle_diff(object_info['ang_hist'][-1],
                                                                                object_info['ang_hist'][-2])/(
                                                                        object_info['time_hist'][-1] -
                                                                        object_info['time_hist'][-2])
            if len(object_info['time_hist']) > 2 and acc_samples > 0:
                acc_samp = max(acc_samples, len(object_info['time_hist']) - 2)
                velocities = [(object_info['pos_hist'][-i] - object_info['pos_hist'][-i - 1])/(
                        object_info['time_hist'][-i] - object_info['time_hist'][-i - 1])
                ...
                ang_velocities = [self.angle_diff(object_info['ang_hist'][-i], object_info['ang_hist'][-i - 1])/(
                        object_info['time_hist'][-i] - object_info['time_hist'][-i - 1])
                                  for i in range(1, 3)]
                self.blimps[obj_id]['info']['angular']['acc'] = (ang_velocities[-1] - ang_velocities[-2])/(
                        object_info['time_hist'][-1] - object_info['time_hist'][-2])
