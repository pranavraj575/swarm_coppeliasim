...
class Waypoints:
    ...
    def talker(self):
        ...
        datasize = 4
        height = 1
        weight = [1.0, 1.0, 1.0, 1.0]       #[goal,obs,avoid,cohese]
        obstacle = [[-3.0, 0, height],
                    [-3.0, 0, height],
                    [-3.0, 0, height]]
        initial_speed = 1
        max_speed = 1.25
        ...
        while not rospy.is_shutdown():
            ...
            #####################################################################################################
            # Viscosity
            v_avoid = Vector3f.Vector3(0,0,0)
            v_cohese = Vector3f.Vector3(0,0,0)
            count = 0
            if self.data.data:
                for x in range(0,len(self.data.data),datasize):
                    if self.data.data[x] != float(self.lta3_id.replace("lta", "")):
            ...
            #####################################################################################################
            # Full
            v_full = v_goal * weight[0] + v_obs_tot * weight[1] + v_avoid * weight[2] + v_cohese * weight[3]
            ...
            if self.data.data:
                for z in range(0,len(w.data),datasize):
                    ...
                    if w.data[z] == float(self.lta3_id.replace("lta", "")):
