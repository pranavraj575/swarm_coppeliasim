...
def eval_genom(exp_maker,
    ...
    exp: blimpNet = exp_maker(net=genome, sim=None, port=port, wakeup=None)
...
class CompGeneralEvolutionaryExperiment:
    def __init__(self,
                 checkpt_dir,
                 exp_maker,
                 config_file):
...
class CompEvolutionExperiment(CompGeneralEvolutionaryExperiment):
    ...
    def result_of_experiment(self, trials=1, gen_indices=(-1,), network_to_use=None, show_random=False, display=True, zmqport=23000,
        ...
        for index in gen_indices:
            if network_to_use is None:
                ...
                if show_random:
                    genomes=[p.population[g] for g in p.population]
                    genome=genomes[np.random.randint(0,len(genomes))]
                else:
...
class EcosystemEvolutionExperiment(CompGeneralEvolutionaryExperiment):
