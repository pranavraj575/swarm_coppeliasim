...
class MAV1_PID(object):
    ...
    def __init__(self):
        ...
        self.PIDs = [[5, 0, 5], [2, 0, 2], [2, 0, 3], [2, 0, 3]]    # Default values
        # self.PIDs = [[5, 0, 5], [5, 0, 2], [2, 0, 3], [2, 0, 3]]  # Loy values for yaw
        #what should we initialize to?
    ...
    def generate_desired(self,state,ref):
        ...
        self.forward.setSetPoint(ref.pose.pose.position.x)
        self.lateral.setSetPoint(ref.pose.pose.position.y)
        self.altitude.setSetPoint(ref.pose.pose.position.z)
        ...
        print "Yaw: " + str(yaw)
        yawcorrection = -1* .8 *self.yaw.update(yaw) #-1 because the motor mapping is currently wrong.
        forwardcorrection = .5* self.forward.update(state.pose.pose.position.x)
        lateralcorrection = self.lateral.update(state.pose.pose.position.y)
        altitudecorrection = self.altitude.update(state.pose.pose.position.z)
        ...
        des.twist.linear.x = forwardcorrection
